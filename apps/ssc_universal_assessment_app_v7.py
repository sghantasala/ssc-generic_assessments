...
# ===========================
st.set_page_config(
    page_title="SSC Universal Assessment App (V7)",
    layout="wide"
)
...
CONFIG_CANDIDATES = [
    "config/assessments_config.json",   # Recommended path
...
# Fallback defaults if no config is found at all
...
def resolve_template_path(cfg: dict, template_key: str):
    ...
    if not template_key:
        return None
    assessments = cfg.get("assessments", {})
    info = assessments.get(template_key)
    if not info:
        return None
...
def main():
    ...
    with logo_cols[1]:
        title_html = (
            ...
            "SSC Universal Assessment App (V7)</h1>"
        )
        subtitle_html = (
    ...
    st.markdown(
        ...
        SSC Universal Assessment App (V7) - StrategyStack Consulting 2025
        </div>
        """,
        unsafe_allow_html=True,
    )

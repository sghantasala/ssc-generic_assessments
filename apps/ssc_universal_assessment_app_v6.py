...
# ---------------------------
st.set_page_config(
    page_title="SSC Universal Assessment App (V6)",
    layout="wide"
)
...
DEFAULT_CLIENT_LOGO_PATH = "logos/client_logo.png"  # optional generic client logo

# Map of template keys (from URL) to Excel files in the repo
TEMPLATE_MAP = {
    # Example mappings – update filenames as you add templates
    "bain_p2p": "templates/bain_p2p_coe_assessment_v1.xlsx",
    "jmcl_trq": "templates/jmcl_trq_readiness_v1.xlsx",
    "ringc_price": "templates/ringc_pricing_margin_v1.xlsx",
    # Fallback generic key
    "generic": "templates/ssc_assessment_template_generic.xlsx",
}

# Map of client keys (from URL) to logo files
CLIENT_LOGO_MAP = {
    "bain": "logos/bain_logo.png",
    "jmcl": "logos/jmcl_logo.png",
    "ringc": "logos/ring_concierge_logo.png",
    # Add more as needed
}

# Optional mapping from client key to friendly display name (used in future enhancements)
CLIENT_DISPLAY_NAME = {
    "bain": "Bain Global Resources",
    "jmcl": "J.McLaughlin",
    "ringc": "Ring Concierge",
}
...
def ai_pre_score_responses(assessment_name: str, client_responses: dict) -> dict:
    ...
    return ai_scores = {dimension: [{"index":..,"question":..,"answer":..,"ai_score":int,"reason":str,
                                     "weight":..,"help_text":..}, ...]}
...
def read_routing_params():
    ...
    template_key = qp.get("template", [""])[0].strip().lower() if qp.get("template") else ""
    client_key = qp.get("client", [""])[0].strip().lower() if qp.get("client") else ""
...
def select_template_file(uploaded_file):
    """
    Decide which template to use, in this priority order:
    1) User-uploaded Excel (sidebar)
    2) Template from URL routing (?template=key)
    3) Default generic template(s)
    """
    # 1) Uploaded file always overrides everything
    if uploaded_file is not None:
        return uploaded_file

    # 2) Check template key from URL routing
    template_key = st.session_state.get("template_key", "").strip().lower()
    if template_key:
        mapped_path = TEMPLATE_MAP.get(template_key)
...
def render_sidebar():
    ...
    uploaded_template = st.sidebar.file_uploader("Upload Assessment Template (.xlsx)", type=["xlsx"])

    template_file = select_template_file(uploaded_template)
...
def main():
    ...
    client_key = st.session_state.get("client_key", "")
    client_logo_path = CLIENT_LOGO_MAP.get(client_key, DEFAULT_CLIENT_LOGO_PATH)

    with logo_cols[1]:
        st.markdown(
            "<h1 style='text-align:center;color:#002060;font-family:Segoe UI, sans-serif;'>"
            "SSC Universal Assessment App (V6)</h1>",
            unsafe_allow_html=True,
        )
        st.markdown(
            "<h4 style='text-align:center;color:gray;font-family:Segoe UI, sans-serif;'>"
            "Client Self-Assessment + AI Pre-Scoring + Consultant Override + URL-Based Routing</h4>",
            unsafe_allow_html=True,
        )
    ...
    st.markdown(
        ...
        SSC Universal Assessment App (V6) • StrategyStack Consulting © 2025
        </div>
        """,
        unsafe_allow_html=True,
    )
